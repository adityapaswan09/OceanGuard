...
const severityStyles = {
    HIGH: "border-ember/30 bg-panelAlt/80 text-ember",
    MEDIUM: "border-[#ecd6a2] bg-panelAlt/80 text-[#9a711d]",
    INFO: "border-[#b9dce7] bg-panelAlt/80 text-signal",
};
...
export function AlertsPage({ onOpenAlert }: AlertsPageProps) {
    ...
    return <main className="min-w-0 flex-1 overflow-auto bg-panel">
        <div className="border-b border-line bg-panel/90 px-5 py-5 lg:px-8">
            <div className="flex items-end justify-between gap-4">
                <div><div className="mb-2 flex items-center gap-2 text-[10px] text-mist"><span>Alerts</span><span>/</span><span className="text-signal">Regional monitoring</span></div><h1 className="font-display text-2xl font-semibold text-ink">Alerts</h1><p className="mt-1 text-xs text-mist">Investigation events for the Arabian Sea workspace. Demo events for prototype review.</p></div>
        ...
        <div className="max-w-5xl px-5 py-6 lg:px-8">
            ...
            {demoAlerts.length === 0 ? <div className="border border-line bg-panel/90 px-4 py-8 text-center text-xs text-mist">No alerts for this region.</div> : <div className="border border-line bg-panel/90 panel-shadow">{demoAlerts.map((alert, index) => { const selected = selectedAlertId === alert.id; return <button className={`flex w-full items-start gap-4 border-b border-line px-4 py-4 text-left last:border-b-0 ${selected ? "bg-panelAlt/80" : "hover:bg-panelAlt/80"}`} key={alert.id} onClick={() => { setSelectedAlertId(alert.id); if (alert.spillId) onOpenAlert(alert); }} type="button"><span className={`mt-0.5 min-w-[58px] rounded-sm border px-2 py-1 text-center text-[9px] font-semibold tracking-[.08em] ${severityStyles[alert.severity]}`}>{alert.severity}</span><span className="min-w-0 flex-1"><span className="block text-xs font-semibold text-ink">{alert.title}</span><span className="mt-1 block max-w-2xl text-[11px] leading-5 text-mist">{alert.description}</span><span className="mt-2 block font-mono text-[9px] text-mist">{alert.timestamp}{alert.spillId ? <span className="ml-3 text-signal">SPILL MS-{String(alert.spillId).padStart(3, "0")}</span> : null}</span></span><span className={`mt-1 text-xs ${index === 0 || index === 1 ? "text-signal" : "text-transparent"}`}>●</span></button>; })}</div>}
        </div>
    </main>;
}
